...
import { calcTune, VALID_TUNES } from './calcTune';
...
/**
 * Check if a car supports a specific tune.
 * With the calculated tune system, all valid tunes are available for every car.
 */
export const hasUpgrade = (car, upgrade) => {
	if (!car) return false;
	return VALID_TUNES.includes(upgrade);
};

/**
 * Get car stats for a selected tune using the calculated tune system.
 * Computes all 6 stats (topSpeed, 0to60, handling, weight, mra, ola) via calcTune.
 */
export const getCarStats = (car, upgrade = "000") => {
	if (!car) return null;
	const tuned = calcTune(car, upgrade);
	return {
		topSpeed: tuned.topSpeed, "0to60": tuned.accel, handling: tuned.handling,
		weight: tuned.weight, mra: tuned.mra, ola: tuned.ola, gc: car.gc,
