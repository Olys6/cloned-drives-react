...
const CarFilter = ({
  ...
  const getAllBodyStyles = () => {
    carData.forEach((car) => {
      if (Array.isArray(car.bodyStyle) && car.bodyStyle !== undefined) {
        car.bodyStyle.forEach((body) => {
          if (!bodyStyles.includes(body)) {
            bodyStyles.push(body);
          }
        })
      } else {
        if (!bodyStyles.includes(car.bodyStyle) && car.bodyStyle !== undefined) {
